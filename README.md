...
Для нового режима достаточно назвать изображения по порядку сцен:

```text
01_первая сцена.jpg
02_вторая сцена.webp
03_третья сцена.png
```
...
## Ручные временные метки

Если временные метки присутствуют у всех изображений, AutoFrames автоматически переключается в прежний точный ручной режим. Смешивать обычные имена и ручные метки в одном проекте нельзя: такая загрузка отклоняется целиком без частичного добавления файлов.
...
- естественная сортировка обычных имён (`scene2` раньше `scene10`);
- ручной режим по временным меткам и непрерывный таймлайн;
